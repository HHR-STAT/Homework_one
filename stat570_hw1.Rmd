...
The estimated beta0 is `r summary$coefficients[1,1]`, estimated beta1 is `r summary$coefficient[2]`. The estimated equation is y = `r summary$coefficients[1,1]` + `r summary$coefficients[2,1]` $\times$ x The 95% confindence interval for beta1 is (`r summary$coefficients[2,1] - 1.96 * summary$coefficients[2,2]`, `r summary$coefficients[2,1] + 1.96 *summary$coefficients[2,2]`), which captures the true beta1. The RSE is `r summary$sigma` on 38 d.f., which is reasonably close to 2.

#### b. i) Generate 100 samples of size 40, and estimate the slope coefficient from each sample.

We wrote a function "simlm" to generate samples, this function returns a table of the estimated parameters and CIs for the 100 trials.
...
Below is summary of simulated beta 1.
```{r}
beta1 = samp[,2]
#summarize beta 1
summary(beta1)
...
The distribution of beta1 seems to center around `r mean(beta1)`, which is pretty close to the true beta1 (1.2).

#### iii) How many times do your 95% confidence intervals cover the true β1 ? 
__Display your confidence intervals graphically. [Optional: Is there any noticeable relation
between the length of the interval and whether or not it covers the true β1 ? – You may want to collect more than 100 samples to investigate this.]__

There are about 97 times that my 95% CI convered the true beta1 in this simulation.
...
#The input argument "samp" is a simlm object. The value of true b1, used in the original simulation, should also be provided.
  ...
  par(new=T)
  plot(x,bi_ub, ylim=c(min(bi_lb),max(bi_ub)), col="green",cex=0.5,ylab="CI", xlab="trial number")
...
numsim = 100
simsize = seq(numsim)
result = data.frame(matrix(data=NA, ncol=2,nrow=100))
for (i in 1:numsim){
...
plot(simsize, result[,2], col="blue", cex=1, xlab="Sample size/10", ylab="Percentage of CI's that fail to capture the true mean")

result = result[order(result[,2]),]
plot(result[,1], result[,2], col="blue", cex=1, xlab="Length of CI", ylab="Percentage of CI's that fail to capture the true mean")
abline(lm(result[,2]~result[,1]), col="red")

fitci = lm(result[,2]~result[,1])
summary(fitci)

```
The result appears to be pretty random. Therefore there isn't a relation between sample size and whether the CI derived from the sample could capture true mean. And the second plot shows that there is essentially no correlation between the length of CI and the percentage of CI's that fails to capture the true mean. This is also confirmed by the regression model, whose p-value is about 0.5.
...
__Question: Summarize the data by providing suitable summary statistics and graphs. Write a brief report for your findings.__

* Load and examine the variables in our  dataset:
...
"team","tot.payroll","avgwin","payroll.1998","pctwin.1998","payroll.2006","pctwin.2006","payroll.2014","pctwin.2014"

* Create a new row of averages for each variable selected across all teams, and label it "average"
...
* Since we are interested in Boston Red Sox and Oakland A, below we examine the observations for these two teams and total average. 

```{r display_rows}
# display the observations for the teams we are interested in (Boston Red Sox & Oakland A's), plus the average across all teams  
print(paydata1[c(4,20,31),])
```

To summarize data, we looked at the following aspects:
* Across teams, we looked at the distribution of total payroll. It suggests that New York Yankees is an outlier, they pay their players much more than all other teams (on average over 2.5 billions) . 

```{r boxplot_data, fig.height=3, fig.width=3}
##### boxplots for tot.payroll and avgwin 
boxplot(paydata$payroll,
          ylab = "payroll (in billions)",
          main = "Total payrolls,1998 - 2014")
```

* Across teams, we looked at the distribution of average win.

```{r boxplot_data2, fig.height=3, fig.width=3}
boxplot(paydata$avgwin,
          ylab = "fraction of all games won", 
          main = "Average win, 1998 - 2014")
```

* We examine if there's a relationship between total pay roll and average winning percentage. Below is a scatterplot showing winning percentage by total payroll from 1998 to 2014
```{r scatterplot, fig.height=3, fig.width=3}
p1 <- ggplot(paydata, aes(x = payroll, y = avgwin)) + geom_point() + xlab("Total Payroll") + ylab("Win percentage") +  ggtitle("Overall win percentage vs. total payroll") + geom_smooth(method = "lm", se = TRUE)
p1
```

#### ii) Payroll ~ winning percentage
__Question: For a given year, is payroll a significant variable to predict the winning percentage of that year? Choose a year and run a regression to examine this. You may try this for a few different years. Explain your results.__

To answer this question, we ran a regression of the relationship between payroll and win percentage for 1998, 2006, and 2014

* 1998

We fit a regression using payroll in 1998 to predict winning percentage in 1998. 
...
Payroll in 1998 significantly predicts the winning percentage in the same year (p < 0.01). 

```{r regression_line2}
p1998 <- ggplot(paydata1, aes(x = payroll.1998, y = pctwin.1998)) + geom_point() + xlab("Payroll") + ylab("Win percentage") +  ggtitle("Win percentage vs. total payroll in 1998") + geom_smooth(method = "lm", se = TRUE)
p1998
```

* 2006

We fit a regression using payroll in 2006 to predict winning percentage in 1998. 
...
Payroll in 2006 significantly predicts the winning percentage in the same year (p < 0.01). 

```{r regression_line4}
p2006 <- ggplot(paydata1, aes(x = payroll.2006, y = pctwin.2006)) + geom_point() + xlab("Payroll") + ylab("Win percentage") +  ggtitle("Win percentage vs. total payroll in 2006") + geom_smooth(method = "lm", se = TRUE)
p2006
```
...
Payroll in 2014 does Not predict the winning percentage in the same year (p < 0.11). 

```{r regression_line4}
p2014 <- ggplot(paydata1, aes(x = payroll.2014, y = pctwin.2014)) + geom_point() + xlab("Payroll") + ylab("Win percentage") +  ggtitle("Win percentage vs. total payroll in 2014") + geom_smooth(method = "lm", se = TRUE)
p2014
```
